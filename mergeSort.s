...
mergeSort:
	...
	;;  if rsi = rdx {
	cmp	rsi, rdx	; compare start and end, possibly flipped
	je	done
	;;  prep for first mergeSort
	push	rsi
	push	rdx
				; keep end
	add	rdx, rsi	; add start and end
	sar	rdx, 1
	push	rdx			; divide by 2 to get middle
	call	mergeSort
	;; prep for second mergeSort
	pop	rsi
	pop	rdx
	push	rdx		; keep rdx for later
	push	rsi		; keep rsi for later, before incrementing
	inc	rsi
	call	mergeSort
	;;  prep for merge
	pop	rdx		; empty the stack back to where it was
	pop	rcx		; before and put rdx, rcx, and rsi
	pop	rsi		; in the correct locations to call merge
	call	merge
